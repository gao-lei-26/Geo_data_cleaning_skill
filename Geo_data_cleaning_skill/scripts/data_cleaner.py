...
class GeoDataCleaner:
    ...
    def clean_age(self):
        ...
        if age_choice and age_choice in self.df_main.columns:
            def parse_age(val):
                ...
                if 'Ga' in s:
                    ...
                    try: 
                        ...
                        return np.nan
            self.df_main['cleaned_age_Ma'] = self.df_main[age_choice].apply(parse_age)
            ...
            self.df_main['cleaned_age_Ma'] = self.df_main[age_cols].apply(
                lambda row: np.nanmean([parse_age(v) for v in row if not pd.isna(v)]), axis=1
            )
            self.log.append(f"年龄标准化: 多个年龄列取平均 -> cleaned_age_Ma")
